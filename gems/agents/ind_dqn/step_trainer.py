...
class Trainer(object):
        ...
        while global_step < max_global_steps:
            ...
            for step in xrange(max_step_per_ep):
                ...
                # for i in xrange(n_agents):
                    # exp = [obs_n[i], act_n[i], reward_n[i], obs_n_next[i], done]
                    # self.agent_singleton.add_to_memory(exp, i)                

                exp = [obs_n, act_n, reward_n, obs_n_next, done_n]
                ...
                if global_step % test_interval == 0:
                    ...
                    print global_step, ep, "%0.2f"%(mean_steps), mean_b_reward, "%.2f"%(mean_b_reward.mean()), "%.2f"%(success_rate.mean()),  "%0.2f"%epsilon
    ...
    def pred_log(self, file, episode):
        
        file.write("episode\t" + str(episode) +"\n")
        fs = self.env.get_full_encoding().flatten()
        file.write("f_state\t")
        file.write(stringify(fs) + "\n")