from agents.qmix.agent import Agent
import numpy as np
...
class Trainer(object):
    def __init__(self, environment, logger):
        ...
        self.agent_singleton = Agent(global_state_space=len(self.env.get_global_state()),
                              act_space=self._agent_profile["agent"]["act_spc"],
    ...
    def get_actions(self, obs_n, epsilon=0.0):
        ...
        act_n = self.agent_singleton.act_multi(obs_n, random)
        return act_n
    ...
    def learn(self, max_global_steps, max_step_per_ep):
        ...
        while global_step < max_global_steps:
            ...
            for step in xrange(max_step_per_ep):
                ...
                exp=[obs_n, act_n, reward_n.mean(), obs_n_next, done,
                     global_state, next_global_state]
                self.agent_singleton.add_to_memory(exp)

                if global_step > steps_before_train and global_step % train_interval == 0:
                    td = self.agent_singleton.train()
                    tds.append(td)

                if global_step % test_interval == 0:
                    
                    mean_steps, mean_b_reward, success_rate, \
                        global_success_rate, squared_error = self.test(80, max_step_per_ep)
                    ...
                    self.logger.info("%d\tb_rwd\t%s" %(global_step, stringify(mean_b_reward[:n_agents],"\t")))
                    self.logger.info("%d\tsuccs\t%s" %(global_step, stringify(success_rate[:n_agents], "\t")))
    ...
    def test(self, max_ep, max_step_per_ep, max_steps=10000, verbose=False):
        if max_steps < max_step_per_ep:
            max_steps = max_step_per_ep
        ...
        return mean_steps, mean_b_reward, success_rate, global_success_rate, squared_error