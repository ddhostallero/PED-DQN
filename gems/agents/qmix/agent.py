from agents.qmix.dq_network import DQN
...
class Agent(object):
    def __init__(self, global_state_space, obs_space, act_space, n_agents, sess, name):
        self.act_space = act_space
        self.obs_space = obs_space
        self.n_agents = n_agents

        self.dqn = DQN(sess, global_state_space, obs_space, act_space, n_agents, name)
    ...
    def train(self):
        ...
        td_error,_ = self.dqn.training_qnet(global_state, state, action, 
            reward, not_done, next_global_state, next_state)
