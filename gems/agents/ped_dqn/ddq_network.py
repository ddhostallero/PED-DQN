...
class DQN:
    def __init__(self, sess, state_dim, action_dim, n_agents, nn_id):
        ...
        self.mission = BaseDQN(sess, state_dim, action_dim, n_agents, "lng"+nn_id, use_as_peer=True)
        # self.mission = BaseDQN(sess, state_dim, action_dim, n_agents, "lng"+nn_id)

    def training_target_qnet(self):
        self.sess.run([self.action.update_slow_target_dqn, self.mission.update_slow_target_dqn])

    def training_peer_qnet(self):
        self.sess.run(self.mission.update_peer_dqn)
