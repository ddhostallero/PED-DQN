        ...
        while global_step < max_global_steps:
            ...
            for step in xrange(max_step_per_ep):
                ...
                # if global_step > steps_before_train and global_step % (train_interval//2) == 0:
                    # self.agent_singleton.train_long_dqn()

                if global_step > steps_before_train and global_step % train_interval == 0:
                    mtd = self.agent_singleton.train_long_dqn()
                    mtds.append(mtd)

                    td, up, rx = self.agent_singleton.train(global_step>50000)
                    tds.append(td)   
                    rxs.append(rx)

                    if up:
                        self.logger.info("%d\tpeeru" %(global_step))
                ...
                if global_step % test_interval == 0:
                    ...
                    mtd = np.asarray(mtds).mean(axis=0)
                    rx = np.asarray(rxs).mean()
                    self.logger.info("%d\ttd_er\t%s" %(global_step, stringify(td, "\t")))
                    self.logger.info("%d\tmtder\t%s" %(global_step, stringify(mtd, "\t")))
