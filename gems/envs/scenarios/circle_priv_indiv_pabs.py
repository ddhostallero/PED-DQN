...
class Agent(AgentBase):
    ...
    def base_reward(self, terminal):
        link_use = -1*np.abs(self.prev_has - self.n_has)
        self.prev_has = self.n_has
        return link_use + (-1*self.get_abs_error())
...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world, args=None):
        super(Scenario, self).reset_world(world, args)

        for agent in world.agents:
            agent.prev_has = agent.n_has